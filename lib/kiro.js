 ...
 *
 * Requires:
 *   - `kiro-cli` binary in PATH (https://docs.kiro.dev/)
...
export async function processKiro(page, email, password, log = console.log, opts = {}) {
    ...
    // Pre-flight: verify kiro-cli is reachable before spawning a long-lived
    // child process.  A missing binary would otherwise emit an async 'error'
    // event whose rejection is not yet awaited — crashing the Node process.
    try {
        await execFileP('kiro-cli', ['--version'], { timeout: 5000 });
    } catch (e) {
        if (e.code === 'ENOENT') {
            throw new Error(
                'kiro-cli not found in PATH. Install it first: https://docs.kiro.dev/'
            );
        }
