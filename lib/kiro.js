...
export async function processKiro(page, email, password, log = console.log, opts = {}) {
    ...
    // Pre-flight: resolve kiro-cli binary.  If missing, auto-install it.
    // Without this check the spawn() below emits an async 'error' event whose
    // rejection isn't awaited yet — crashing Node.
    const isWin = process.platform === 'win32';
    // Windows MSI installs to %LOCALAPPDATA%\Kiro-Cli\
    // Linux/macOS installer installs to ~/.local/bin
    const installDir = isWin
        ? path.join(process.env.LOCALAPPDATA || path.join(homedir(), 'AppData', 'Local'), 'Kiro-Cli')
        : path.join(homedir(), '.local', 'bin');
    const binaryName = isWin ? 'kiro-cli.exe' : 'kiro-cli';

    // Resolve the full path: try PATH first, then the known install directory.
    let kiroBin = binaryName;
    function resolveKiroBin() {
        const fullPath = path.join(installDir, binaryName);
        if (existsSync(fullPath)) return fullPath;
        return binaryName; // fall back to bare name (PATH lookup)
    }

    try {
        await execFileP(kiroBin, ['--version'], { timeout: 5000 });
    } catch (e) {
        if (e.code === 'ENOENT') {
            // Check known install dir before downloading.
            kiroBin = resolveKiroBin();
            let alreadyInstalled = false;
            if (kiroBin !== binaryName) {
                try {
                    await execFileP(kiroBin, ['--version'], { timeout: 5000 });
                    alreadyInstalled = true;
                    log(`[+] Found kiro-cli at ${kiroBin}`);
                } catch { /* not usable, proceed with install */ }
            }

            if (!alreadyInstalled) {
                const installCmd = isWin
                    ? "powershell -NoProfile -Command \"irm 'https://cli.kiro.dev/install.ps1' | iex\""
                    : 'curl -fsSL https://cli.kiro.dev/install | bash';
                log(`[*] kiro-cli not found — auto-installing (${isWin ? 'Windows' : 'Linux/macOS'})...`);
                try {
                    const { stdout } = await execP(installCmd, {
                        timeout: 120_000,
            ...
            // Update PATH so child processes also find it.
            const sep = isWin ? ';' : ':';
            const currentPath = process.env.PATH || '';
            if (!currentPath.split(sep).includes(installDir)) {
                process.env.PATH = `${installDir}${sep}${currentPath}`;
            }
            // Verify with the resolved path.
            try {
                await execFileP(kiroBin, ['--version'], { timeout: 5000 });
    ...
    const child = spawn(kiroBin, ['login', '--license', 'free', '--use-device-flow'], {
