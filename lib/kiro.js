...
export async function processKiro(page, email, password, log = console.log, opts = {}) {
    const manualLogin = !!opts.manualLogin;
    // Manual mode gives the operator time to solve captchas / 2FA in the
    // visible browser window. Auto mode uses the shorter Playwright timeout.
    const TIMEOUT = opts.timeout || (manualLogin ? 600000 : 90000);
    const { codeVerifier, codeChallenge } = generatePkcePair();
    const authState = { authCode: null };

    log('[*] Starting direct Kiro OAuth (PKCE, no kiro-cli)...');
    if (manualLogin) {
        log('[*] MANUAL LOGIN MODE: complete Google login in the Camoufox window.');
        log(`[*] Bot will wait up to ${Math.round(TIMEOUT / 1000)}s for OAuth callback.`);
    }
    ...
    try {
        ...
        if (!authState.authCode) {
            if (manualLogin) {
                // Skip the automated Google login entirely; rely on the operator
                // driving the Camoufox window. The redirect-capture listener
                // installed above will pick up the kiro:// code once Google
                // hands the user back.
                log('[*] Waiting for operator to complete Google sign-in...');
            } else {
                try {
